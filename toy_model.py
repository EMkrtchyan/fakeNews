...
# with open('./textNormalizing/news_cut1300.txt', 'r', encoding='utf-8') as f:
#     text = f.read()
# print("text read")
...
for iter in range(max_iters):
    ...
    if (iter % eval_interval == 0 or iter == max_iters - 1 )and not iter==0:
        ...
        print(f"step {iter}: train loss {losses['train']:.4f}, val loss {losses['val']:.4f}, time:{dt*100:.2f}")
        torch.save(model.state_dict(), f"./weights/bert1300/{iter}.pt")

    # sample a batch of data
    xb, yb = get_batch('train')
    if iter % 100 == 0:
        print(f"iter:{iter}, time:{dt*100:.2f}")
    # evaluate the loss
...
#torch.save(model.state_dict(), "./weights/bert10000.pt")
